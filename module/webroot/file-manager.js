class FileManager {
	...
	async getDirectoryContents() {
		...
		try {
			...
			const {
				stdout: dirs
			} = await exec(`find "${this.currentPath}" -maxdepth 1 -type d -not -path "${this.currentPath}" 2>/dev/null | sort`);
			if (dirs.trim()) {
				dirs.split('\n').forEach(dirPath => {
			...
			if (this.mode === 'image') {
				const {
					stdout: images
				} = await exec(`find "${this.currentPath}" -maxdepth 1 -type f -name "*.img" 2>/dev/null | sort`);
				if (images.trim()) {
					images.split('\n').forEach(imgPath => {
